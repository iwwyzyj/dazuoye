...
原因：Sealos 内网地址可能不是简单的 `dazuoye-backend`，或后端尚未 Running。

处理：
1. 先确保 **dazuoye-backend** 状态为 Running
2. 在后端详情页复制 **内网地址**（例如 `http://dazuoye-backend-xxxxx.ns-xxxxx:8080`）
3. 在前端 **高级配置 → 环境变量** 添加：

```
BACKEND_URL = http://dazuoye-backend-xxxxx.ns-xxxxx:8080
```

（把地址换成你后端详情页里显示的内网地址，保留 `http://`，不要末尾斜杠）
